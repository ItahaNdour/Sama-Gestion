...
const state={logged:false,workspace:"global",agents:[],clients:[],properties:[],payments:[],visits:[],edls:[],filter:"Tous",trackingId:null};
let photos=[];
...
function ensure(){if(!state.agents.length)state.agents=[{id:"main",name:"Agence principale",role:"Agence",phone:"221770000000",email:"admin@immohub.sn",signature:"Agence principale\nWhatsApp : 221770000000"}]}
...
function login(){ $("#loginScreen").classList.add("hidden"); $("#app").classList.remove("hidden"); state.logged=true; save(); render(); }
function logout(){ state.logged=false; save(); $("#loginScreen").classList.remove("hidden"); $("#app").classList.add("hidden"); }
...
function openModal(id){resetForms(); $("#"+id).classList.add("open"); if(id==="paymentModal") prefillPayment();}
...
function render(){
  ...
  $("#dueList").innerHTML=due.length?due.map(p=>`<div class="card"><p>⏰ ${monthLabel(p.month)} • ${prop(p.propertyId)?.name||"Bien"} • reste ${money(p.remaining)}</p></div>`).join(""):'<div class="card"><p>Aucune échéance en attente.</p></div>';
...
function renderAgents(){$("#agentsList").innerHTML=state.agents.map(a=>`<article class="card"><div class="card-top"><div><h3>👤 ${a.name}</h3><p>${a.role} • ${a.email||""}</p></div><span class="badge orange">${state.properties.filter(p=>p.agentId===a.id).length} biens</span></div><p>📱 ${a.phone||"-"}</p><p>Signature : ${a.signature||a.name}</p><div class="actions"><button class="mini-btn blue" onclick="editAgent('${a.id}')">Modifier</button>${a.id!=="main"?`<button class="mini-btn red" onclick="del('agents','${a.id}')">Supprimer</button>`:""}</div></article>`).join("");}
...
function editAgent(id){let a=agent(id); openModal("agentModal"); $("#agentId").value=a.id; $("#agentName").value=a.name; $("#agentPhone").value=a.phone; $("#agentEmail").value=a.email; $("#agentRole").value=a.role; $("#agentSignature").value=a.signature||""}
...
function prefillPayment(){const p=prop($("#paymentProperty").value); if(!p)return; const now=new Date(); const ym=now.toISOString().slice(0,7); $("#paymentMonth").value=$("#paymentMonth").value||ym; $("#paymentClient").value=p.occupantId||""; $("#paymentRent").value=p.price||0; $("#paymentManagementRate").value=p.managementRate||0; $("#paymentMoveInDate").value=p.moveInDate||""; calculatePayment();}
...
function bind(){
  ...
  $("#demoBtn").onclick=seed; $("#resetBtn").onclick=()=>{if(confirm("Tout effacer ?")){localStorage.removeItem(KEY);location.reload()}};
  ...
  $("#agentForm").onsubmit=e=>{e.preventDefault();let id=$("#agentId").value||uid("agent"), a={id,name:$("#agentName").value,phone:$("#agentPhone").value,email:$("#agentEmail").value,role:$("#agentRole").value,signature:$("#agentSignature").value}; let i=state.agents.findIndex(x=>x.id===id); i>=0?state.agents[i]=a:state.agents.push(a); save(); closeModals(); render()};
...
load();ensure();bind(); if(state.logged) login(); else render();
