...
// GESTION DE L'ÉTAT DE CONNEXION INITIALE (RECONNAISSANCE AUTOMATIQUE)
window.onload = () => {
    window.fbOnAuth(window.auth, async (user) => {
        if (user) {
            courtierEmail = user.email;
            
            // Si c'est ton adresse, pas de question : tu es le SuperAdmin 👑
            if (user.email === "19amadoundour@gmail.com") {
                profilRole = "SuperAdmin";
                courtierNom = "Amadou ND";
                monAvatar = "👑";
                
                // On s'assure que la base de données Firestore est synchronisée avec ces informations
                await window.fsSetDoc(window.fsDoc(window.db, "profils", user.uid), {
                    uid: user.uid, fullname: courtierNom, role: profilRole, avatar: monAvatar, email: user.email
                }, { merge: true });
            } else {
                // Logique standard pour les autres courtiers éventuels
            ...
            // Si aucun utilisateur n'est connecté, on affiche l'écran de connexion
...
// CORRECTION DE LA FONCTION DE CONNEXION (RETOUR À LA MÉTHODE DIRECTE FIREBASE)
async function verifierConnexion() {
    ...
    try {
        // Connexion directe via le module d'authentification Firebase global
        await window.fbSignIn(window.auth, emailSaisi, passSaisi);
    } catch (error) { 
        console.error("Détail de l'erreur Firebase :", error.code, error.message);
        alert("Erreur d'accès : Identifiants incorrects ou projet Firebase non synchronisé."); 
    }
}
